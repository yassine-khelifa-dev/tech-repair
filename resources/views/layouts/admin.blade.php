...
<body>
    <div class="min-h-screen antialiased bg-gray-900 dark:bg-gray-900">
            ...
            <div class="flex flex-wrap justify-between items-center">
                <div class="flex justify-start items-center">
                    ...
                    <a href="{{ route('dashboard') }}" class="flex items-center gap-3 mr-4">
                        <span
                            class="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-600 text-white shadow-lg shadow-blue-900/30 ring-1 ring-blue-400/30">
                ...
                <div class="flex items-center lg:order-2">
                    <button type="button"
                        ...
                        <span
                            class="flex h-9 w-9 items-center justify-center rounded-lg bg-blue-600 text-sm font-bold text-white shadow-lg shadow-blue-900/30">
                            {{ strtoupper(substr(auth()->user()->name ?? 'U', 0, 1)) }}
                        </span>
                        <span class="hidden min-w-0 sm:block">
                            <span
                                class="block truncate text-sm font-semibold text-white">{{ auth()->user()->name ?? 'User' }}</span>
                            <span
                                class="block max-w-56 truncate text-xs text-gray-400">{{ auth()->user()->email ?? 'Email unavailable' }}</span>
                            <span
                                class="block max-w-56 truncate text-xs text-red-400">{{ auth()->user()->role ?? 'Role unavailable' }}</span>
                        </span>
                    ...
                    <div class="hidden z-50 my-4 w-64 overflow-hidden rounded-xl border border-white/10 bg-gray-800 text-base list-none shadow-2xl shadow-black/30"
                        ...
                        <div class="border-b border-white/10 px-4 py-3">
                            <span
                                class="block truncate text-sm font-semibold text-white">{{ auth()->user()->name ?? 'User' }}</span>
                            <span
                                class="mt-1 block truncate text-sm text-gray-400">{{ auth()->user()->email ?? 'Email unavailable' }}</span>
                        </div>
                        <div class="py-1">
        ...
        <aside
                        ...
                        : 'text-gray-600 hover:bg-gray-100 hover:text-gray-950 dark:text-gray-300 dark:hover:bg-gray-700/70 dark:hover:text-white' }}">
                                    ...
                                    <span
                                        ...
                                        <svg class="h-4 w-4" fill="currentColor" viewBox="0 0 20 20"
                                            aria-hidden="true">
                                            <path
                                                d="M2.94 6.34A2 2 0 014.77 5h10.46a2 2 0 011.83 1.34L10 10.77 2.94 6.34z" />
                                            <path
                                                d="M18 8.12V14a2 2 0 01-2 2H4a2 2 0 01-2-2V8.12l7.47 4.68a1 1 0 001.06 0L18 8.12z" />
                                        </svg>
                                    </span>
            ...
            <div class="hidden absolute bottom-0 left-0 w-full bg-white p-4 dark:bg-gray-800 lg:block">
                <div
                    class="rounded-xl border border-amber-400/20 bg-amber-500/10 px-3 py-3 text-amber-100 shadow-lg shadow-black/10">
                    <div class="flex items-center gap-2">
                        <span
                            class="flex h-8 w-8 items-center justify-center rounded-lg bg-amber-400/15 text-amber-200 ring-1 ring-amber-300/20">
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.8"
                                stroke="currentColor" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <div>
                            ...
                            <p class="mt-0.5 text-xs leading-5 text-amber-100/80">Limited build, not ready for client
                                delivery.</p>
                        </div>
                    </div>
        ...
        <main class="p-4 md:ml-64 min-h-screen pt-20 mt-5 bg-gray-900">
            @yield('content')
