...
<form action="{{  route('device-attribute.update', $device_attribute->id) }}" method="post">
    @method('PUT')
    @csrf
 ...
 <div class="border-b border-white/10 pb-12">
    <h2 class="text-base/7 font-semibold text-white">Edit Device Attribute</h2>
    <p class="mt-1 text-sm/6 text-gray-400">Edit a dynamic attribute for a device type.</p>
    ...
    <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-12 sm:grid-cols-3">
            ...
            <select id="device_type_id"
                ...
                @foreach ($devicetypes as $devicetype)
                    ...
                    <option class="bg-black text-white"
                            value="{{ $devicetype->id }}" {{ old('device_type_id', $device_attribute->device_type_id) == $devicetype->id ? 'selected' : '' }} >
        ...
        <div class="sm:col-span-3">
            ...
            <div class="mt-2">
                <input id="name"
                    value="{{ old('name',  $device_attribute->name) }}" placeholder="Storage"
         ...
         <div class="sm:col-span-3">
            ...
            <div class="mt-2">
                <input id="code"
                    value="{{ old('code',  $device_attribute->code) }}" placeholder="Storage"
         ...
         <div class="mt-5 text-white">
            ...
            <select id="input_type"
                ...
                <option class="bg-black text-white" value="{{  null }}" selected>
                    ...
                    <option class="bg-black text-white"value="text"   {{ old('input_type',  $device_attribute->input_type) == "text" ? 'selected' : '' }}>Text </option>
                    <option class="bg-black text-white"value="number"  {{ old('input_type',  $device_attribute->input_type) == "number" ? 'selected' : '' }}> Number</option>
                    <option class="bg-black text-white"value="select" {{ old('input_type',  $device_attribute->input_type) == "select" ? 'selected' : '' }} > Select</option>
                    <option class="bg-black text-white"value="boolean" {{ old('input_type',  $device_attribute->input_type) == "boolean" ? 'selected' : '' }} > Boolean</option>
        ...
        <div class="flex items-center mb-4 text-white">
            <input id="is_required" name="is_required" type="checkbox" value="1" {{  old('is_required',  $device_attribute->is_required ? 'checked' : '') }} class="w-4 h-4 border border-default-medium rounded-xs bg-neutral-secondary-medium focus:ring-2 focus:ring-brand-soft">
            <label for="is_required" name="is_required" class="select-none ms-2 text-sm font-medium text-heading">Required Attribute</label>
        </div>


        <div class="flex items-center mb-4 text-white">
            <input id="is_filterable" name="is_filterable" type="checkbox" value="1" {{  old('is_filterable',  $device_attribute->is_filterable ? 'checked' : '' ) }} class="w-4 h-4 border border-default-medium rounded-xs bg-neutral-secondary-medium focus:ring-2 focus:ring-brand-soft">
         ...
         <div class="sm:col-span-3">
            ...
            <div class="mt-2">
                <input id="sort_order"
                    value="{{ old('sort_order', $device_attribute->sort_order) }}" placeholder="1"
