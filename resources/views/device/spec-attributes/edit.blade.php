...
@section('content')
    <div class="mx-auto max-w-5xl px-4 py-8" x-data="{
        'input_type': {{ Js::from($spec_attribute->input_type) }},
        'attribute_name': {{ Js::from($spec_attribute->name) }},
        'list_size': {{ Js::from($spec_attribute->specOptions->count()) }},
        'code': {{ Js::from($spec_attribute->name) }},
        'unit': '',
        'spec_options': [],
        'spec_options': @js(old('spec_options', $spec_attribute->specOptions->pluck('label')->values()->toArray())),

        addOption() {
            this.spec_options.push('')
        },

        remove(index) {
            if (this.spec_options.length > 1) {
        ...
        <form action="{{ route('spec-attribute.update', $spec_attribute->id) }}" method="POST" class="space-y-8" x-init="init()">
            @method('PUT')
            @csrf

            <div class="rounded-2xl border border-white/10 bg-gray-900/70 shadow-xl">
                <div class="border-b border-white/10 px-6 py-5">
                    <h2 class="text-xl font-semibold text-white">Edit Spec Attribute</h2>
                ...
                <div class="space-y-8 px-6 py-6">
                    ...
                    <div>
                        ...
                        <div class="mt-4 grid grid-cols-1 gap-6 md:grid-cols-2">
                            <div>
                                ...
                                <input id="name" x-model="attribute_name"
                                    value="{{ old('name', $spec_attribute->name) }}" placeholder="Storage" type="text"
                            ...
                            <div>
                                ...
                                <input id="code" x-model="code" value="{{ old('code') }}" placeholder="storage"
                                    type="text" name="code" autocomplete="given-name"
                                    class="mt-2 block w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder:text-gray-500 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
                    ...
                    <div class="rounded-xl border border-white/10 bg-black/20 p-5">
                        ...
                        <div class="mt-4 grid grid-cols-1 gap-6 md:grid-cols-2">
                                ...
                                <select id="input_type" name="input_type"
                                    ...
                                    @foreach (\App\Enums\SpecInputType::cases() as $type)
                                        <option value="{{ $type->value }}"
                                            ...
                                            {{ old('input_type', $spec_attribute->input_type) === $type->value ? 'selected' : '' }}>
                                            {{ $type->value }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="unit" name="unit"
                                    ...
                                    @foreach (\App\Enums\SpecUnit::cases() as $unit)
                                        <option value="{{ $unit->value }}" @click='unit =  @json($unit->value) '
                            ...
                            <div>
                                ...
                                <input id="sort_order" value="{{ old('sort_order', $spec_attribute->sort_order) }}"
                                    placeholder="1" type="text" name="sort_order" autocomplete="given-name"
                                    class="mt-2 block w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder:text-gray-500 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" />
                                <p class="mt-1 text-xs text-gray-500">
                        ...
                        <div class="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
                            <label for="is_required"
                                ...
                                <input id="is_required" name="is_required" type="checkbox" value="1"
                                    {{ old('is_required', $spec_attribute->is_required ? 'checked' : '') }}
                            ...
                            <label for="is_filterable"
                                ...
                                <input id="is_filterable" name="is_filterable" type="checkbox" value="1"
                                    {{ old('is_required', $spec_attribute->is_filterable ? 'checked' : '') }}
                                    class="mt-1 h-4 w-4 rounded border-white/20 bg-gray-900 text-indigo-500 focus:ring-indigo-500">
                                <span>
                    ...
                    <div>
                        ...
                        <div class="mt-4">
                            ...
                            @php
                                $selectedDeviceTypes = old(
                                    'devicetypes',
                                    $spec_attribute->deviceTypes->pluck('id')->toArray(),
                                );
                            @endphp
                ...
                <div class="m-4">
                    ...
                    <template x-for="(option, index) in spec_options" :key="index">
                        <div class="mt-2 flex items-center gap-2">
                            <input :id="'attribute_' + index" x-model="spec_options[index]"
                                x-bind:placeholder="attribute_name" type="text" :name="'spec_options[' + index + ']'"
                            ...
                            <div x-show="['select','multiselect'].includes(input_type)" class="flex gap-2">
            ...
            <div class="flex items-center justify-end gap-4">
                ...
                <button type="submit"
                    class="rounded-lg bg-green-500 px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-indigo-500/20 hover:bg-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-950">
                    Edit Attribute
                </button>
            </div>
        </form>
