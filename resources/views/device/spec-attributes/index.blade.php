...
@section('content')
    <div x-data="{ modal: false, model_selected: null }" class="mx-auto max-w-7xl">
        ...
        <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            @can('create', App\Modles\SpecAttribute::class)
        ...
        <div
            ...
            <div>
                ...
                <p class="mt-1 text-sm leading-6 text-gray-300">
                    Attributes define the fields shown for a device category, such as color, RAM, storage, or screen size.
                    These attributes decide what information can be selected later when configuring a model and creating a
                    repair ticket.
                </p>
            </div>
                            ...
                            <tr class="group transition-colors duration-200 hover:bg-white/[.04]">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div class="mt-1 font-mono text-xs text-gray-500">{{ $spc_attribute->code }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex max-w-xs flex-wrap gap-2">
                                        @foreach ($spc_attribute->deviceTypes->take(3) as $deviceType)
                                            <span
                                                class="inline-flex items-center rounded-full bg-indigo-500/10 px-2.5 py-1 text-xs font-semibold text-indigo-200 ring-1 ring-indigo-400/20">
                                                {{ $deviceType->name }}
                                            </span>
                                        @endforeach
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        <span @class([
                                            ...
                                            'bg-emerald-500/10 text-emerald-200 ring-emerald-400/20' =>
                                                $spc_attribute->is_filterable == 1,
                                            'bg-gray-500/10 text-gray-400 ring-white/10' =>
                                                $spc_attribute->is_filterable != 1,
                                        ])>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-wrap items-center justify-end gap-2">
                                        ...
                                        <!-- Action: Delete -->
                                        @can('delete', $spc_attribute)
                                            <form action="{{ route('spec-attribute.destroy', $spc_attribute->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
                                                ...
                                                <button type="submit"
                                                    ...
                                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                                                        ...
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673A2.25 2.25 0 0115.916 21.75H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                                    </svg>
                                                    Delete
                                                </button>
                                            </form>
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-12 text-center">
                                    <div class="mx-auto flex max-w-sm flex-col items-center">
                                        ...
                                        <p class="mt-1 text-sm text-gray-400">Create the first attribute to start
                                            configuring model specs.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
