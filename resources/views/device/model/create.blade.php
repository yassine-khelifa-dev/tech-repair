        ...
        <div class="overflow-hidden rounded-xl border border-white/10 bg-gray-800/70 shadow-2xl shadow-black/20">
            <div class="border-b border-white/10 px-6 py-5">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-sm font-semibold uppercase tracking-wide text-blue-300">New device model</p>
                        <h1 class="mt-1 text-2xl font-bold text-white">Create Device Model</h1>
                    </div>
                </div>
                <p class="mt-4 text-sm leading-6 text-gray-400">
                    Add a model and connect it to an existing brand and device type.
                </p>
            </div>

            <form action="{{ route('devicemodel.store') }}" method="POST" class="px-6 py-6">
                @csrf
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <select id="brand_id"
                            ...
                            <option class="bg-gray-950 text-white" value="" selected>Choose a brand</option>
                            @foreach ($brands as $brand)
                                <option class="bg-gray-950 text-white" value="{{ $brand->id }}" {{ old('brand_id') == $brand->id ? 'selected' : '' }}>
                                    {{ $brand->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="device_type_id"
                            name="device_type_id"
                            class="mt-2 block w-full rounded-lg border border-white/10 bg-gray-950/60 px-4 py-3 text-sm text-white shadow-sm outline-none transition focus:border-blue-300 focus:ring-2 focus:ring-blue-400/30">
                            <option class="bg-gray-950 text-white" value="" selected>Choose a type</option>
                            @foreach ($devicetypes as $devicetype)
                                <option class="bg-gray-950 text-white" value="{{ $devicetype->id }}" {{ old('device_type_id') == $devicetype->id ? 'selected' : '' }}>
                                    {{ $devicetype->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                    ...
                    <button value="save_create" name="action" type="submit"
                        class="inline-flex items-center justify-center rounded-lg border border-blue-300/30 bg-blue-500/10 px-4 py-2.5 text-sm font-semibold text-blue-100 transition-all duration-200 hover:-translate-y-0.5 hover:bg-blue-500/20 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 focus:ring-offset-gray-800">
                        Save & Create Another
                    </button>
