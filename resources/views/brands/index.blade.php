...
@section('content')
    <h1 class="text-white my-2">Page Brand :</h1>
                     ...
                     <td class="px-6 py-4">
                      <a href="{{ route('brand.edit', $brand->id) }}"
                       ...
                       <form action="{{  route('brand.destroy',    $brand->id ) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <button type="submit" onclick="return confirm('Are you sure to delete this brand? ') " class="rounded-md bg-orange-500 px-3 py-2 text-sm mt-2 font-semibold text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500">
